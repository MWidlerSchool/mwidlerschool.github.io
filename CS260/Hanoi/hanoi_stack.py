...
class HanoiStack():
    """A specialized stack class for the Towers of Hanoi, implemented as a linked list. 
    Only expects to get ints as data members.
    Push, pop, peek, and len are all O(1). list and str are O(n)."""
    ...
    def list(self):
        """Returns the stack as a list, from back to front, for drawing."""
        returnVal = []
        node = self.head
        for i in range(self.size):
            returnVal.append(node.data)
